...
import {
  ...
  Users, ArrowLeft, Monitor, TrendingUp,
  Palette, ShoppingCart, Camera,
  Code2, Settings, LayoutGrid, MessageSquare, Globe, Cpu
} from 'lucide-react'
...
const categories = [
  ...
  { label: 'Stratégie Digitale & Marketing',          value: 'F01',       icon: TrendingUp },
  { label: 'Design & Création Visuelle',              value: 'F02',       icon: Palette },
  { label: 'Développement Web',                       value: 'F03',       icon: Code2 },
  { label: 'E-commerce & Business Digital',           value: 'F04',       icon: ShoppingCart },
  { label: 'No-code & Automatisation',                value: 'F05',       icon: Settings },
  { label: 'SEO & Analyse Web',                       value: 'F06',       icon: Search },
  { label: 'Gestion de Projet Numérique',             value: 'F07',       icon: LayoutGrid },
  { label: 'Création de Contenu',                     value: 'F08',       icon: Camera },
  { label: 'Communication & Rédaction Pro',           value: 'F09',       icon: MessageSquare },
  { label: "Gestion de l'Expérience Digitale",       value: 'F10',       icon: Globe },
  { label: 'Vibe Coding & IA',                        value: 'F11',       icon: Cpu },
  { label: 'Informatique & Bureautique',              value: 'F12',       icon: Monitor },
]
      ...
      <div style={{ background: 'linear-gradient(135deg, #14532d 0%, #166534 60%, #15803d 100%)' }}>
        <div className="max-w-6xl mx-auto px-8 py-16 text-white">
          ...
          <p style={{ color: '#dcfce7' }} className="text-lg mb-8">
            12 formations pratiques, certifiantes et orientées terrain — En ligne & Présentiel à Cotonou.
          </p>
            ...
            {filtered.map((course) => (
              ...
              >
                <div className="h-40 flex items-center justify-center relative" style={{ background: 'linear-gradient(135deg, #14532d, #15803d)' }}>
                  ...
                  <div className="absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold" style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>
                    {course.category || 'F—'}
                  </div>
                  {course.price && (
                    <div className="absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold" style={{ background: 'rgba(255,255,255,0.9)', color: '#14532d' }}>
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4" style={{ borderTop: '1px solid #f3f4f6' }}>
                    <div className="flex items-center gap-3 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><Clock size={12} /> {course.duration || 'Voir détails'}</span>
                      <span className="flex items-center gap-1"><Users size={12} /> Certifié</span>
                    </div>
